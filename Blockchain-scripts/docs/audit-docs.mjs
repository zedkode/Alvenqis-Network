...
const ignoredDirectories = new Set([
  '.agent', '.agents', '.artifacts', '.claude', '.codex', '.cursor', '.fleet', '.gemini',
  '.git', '.idea', '.review', '.alvenqis-local', '.superdesign', '.superdev', '.tmp-firo',
  '.vs', '.vscode', 'InternalAI', 'alvenqis-docker', 'dist', 'node_modules',
  'release-artifacts', 'release-staging', 'target',
])
const ignoredFiles = new Set([
  '00_DECENTRALIZATION_READ_FIRST.md',
  'ALVENQIS_ARCHITECTURE_REDESIGN.md',
...
const ignoredPathPrefixes = [
  'Blockchain-docs/ai/',
  'Blockchain-docs/human/internal/',
  'Blockchain-docs/internal/',
]

async function walk(directory) {
  const results = []
  for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
    if (entry.isDirectory() && (ignoredDirectories.has(entry.name) || entry.name.startsWith('target-'))) continue
