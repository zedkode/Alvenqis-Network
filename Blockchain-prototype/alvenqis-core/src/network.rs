...
impl Network {
    ...
    pub fn from_address_prefix(value: &str) -> Option<Self> {
        // Allowlist only. Any other HRP (including retired ones) is rejected.
        match value {
    ...
    /// True when `value` is not a current Alvenqis network id, HRP, magic, or short alias.
    /// Used to refuse foreign/retired wire identities without listing them in source.
    pub fn is_legacy_identity_token(value: &str) -> bool {
        if value.is_empty() {
            return false;
        }
        if Self::from_network_id(value).is_some() || Self::from_address_prefix(value).is_some() {
            return false;
        }
        !matches!(
            value,
            "devnet" | "testnet" | "mainnet-candidate" | "ALDV" | "ALTS" | "ALMC"
        )
    }
}
...
impl FromStr for Network {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            ...
            _ => Err(crate::AlvenqisError::InvalidNetwork {
...
#[cfg(test)]
mod rebrand_identity_tests {
    use super::*;

    #[test]
    fn allowlists_only_current_alvenqis_identity() {
        assert_eq!(
        ...
        // Unknown / retired prefixes and network ids are rejected (allowlist-only).
        assert!(Network::from_address_prefix("xxxx").is_none());
        assert!(Network::from_network_id("other-mainnet").is_none());
        assert!(Network::is_legacy_identity_token("xxxx"));
        assert!(!Network::is_legacy_identity_token("alve"));
        assert!(!Network::is_legacy_identity_token(
            "alvenqis-mainnet-candidate"
        ));
        assert_eq!(Network::MainnetCandidate.address_prefix(), "alve");
        assert_eq!(
            Network::MainnetCandidate.network_id(),
            "alvenqis-mainnet-candidate"
        );
        assert_eq!(Network::MainnetCandidate.chain_magic_bytes(), *b"ALMC");
    }
}
