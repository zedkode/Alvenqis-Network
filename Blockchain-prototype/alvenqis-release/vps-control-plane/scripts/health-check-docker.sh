...
load_dotenv .env
compose_args
required=("${ALVENQIS_REQUIRED_SERVICES[@]}")
...
rocks_output="$(
  "${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-node \
    alvenqis-node \
...
[[ "$rocks_network" == alvenqis-mainnet-candidate \
  && "$rocks_blocks" -ge 1 \
  && "$rocks_height" -ge 0 \
  && "$rocks_hash" =~ ^[0-9a-f]{64}$ \
  && "$rocks_encryption" == xchacha20poly1305 \
  && "$rocks_key_id" =~ ^[0-9a-f]{16}$ ]] || {
  echo "RocksDB readiness failed: canonical encrypted state is invalid." >&2
  exit 1
}

if compose_has_service alvenqis-rpc; then
  "${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-rpc curl -fsS --max-time 45 http://127.0.0.1:10787/health >/dev/null
  status_json="$("${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-rpc curl -fsS --max-time 45 http://127.0.0.1:10787/status)"
  p2p_json="$("${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-rpc curl -fsS --max-time 45 http://127.0.0.1:10787/p2p/status)"
  ...
  printf 'Live node/RocksDB: height=%s encryption=%s key_id=%s\n' \
    "$rocks_height" "$rocks_encryption" "$rocks_key_id"
fi
...
if compose_has_service gateway; then
  ...
  "${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-rpc curl -fsS -H "Host: ${EXPLORER_HOST}" http://gateway:8080/healthz >/dev/null
fi
if compose_has_service alvenqis-control; then
  "${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-control curl -fsS http://127.0.0.1:10788/health >/dev/null
fi
if compose_has_service alvenqis-metrics-exporter; then
  "${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-metrics-exporter curl -fsS http://127.0.0.1:9101/health >/dev/null
fi
if compose_has_service alvenqis-pool; then
  private_template="$("${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-pool curl -fsS --max-time 20 "http://alvenqis-rpc:10787/mining/template?miner_address=${POOL_ADDRESS}")"
  ...
  pool_health="$("${ALVENQIS_COMPOSE_ARGS[@]}" "${ALVENQIS_PROFILE_ARGS[@]}" exec -T alvenqis-pool curl -fsS --max-time 10 http://127.0.0.1:30787/health)"
