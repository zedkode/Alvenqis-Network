...
```bash
curl -fsSLO https://example.invalid/alvenqis-setup-external.tar.gz
curl -fsSLO https://example.invalid/alvenqis-setup-external.tar.gz.sha256
sha256sum --check alvenqis-setup-external.tar.gz.sha256
sudo install -d -m 0755 /opt/alvenqis
sudo tar -xzf alvenqis-setup-external.tar.gz -C /opt/alvenqis
cd /opt/alvenqis/alvenqis-release/alvenqis-setup-external
```
