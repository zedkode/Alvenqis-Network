...
| ALV-NET-003 | High | `libp2p-yamux 0.47.0` selects vulnerable `yamux 0.12.1` when Alvenqis configures its 32-stream connection cap; GitHub GHSA-vxx9-2994-q338 describes a remotely triggerable panic below `yamux 0.13.10`. | `Blockchain-prototype/alvenqis-node/src/p2p.rs`; both main and keystore Cargo graphs; Dependabot update job `30536896042`; dated dependency-audit report. | Reviewed fixed Yamux adapter that retains bounded streams, malformed-frame regression coverage, multi-node P2P tests, fresh dependency scans, and a successful Dependabot re-check. |
...
| ALV-OPS-001 | High | The project-operated fleet admin surface remains a centralized privileged boundary. Viewer/operator RBAC, agent-controller mTLS, and the local audit hash-chain implementation are deployed, but the live store currently has no audit entries, immutable CI evidence is pending, and the chain head is not externally anchored. | `FLEET_RBAC_MTLS_STATUS_2026-08-01.md`; `AUDIT_LOG_HASH_CHAIN_STATUS_2026-08-08.md`; `TM-1212`; `TM-1216`. | Green immutable CI, non-empty deployed-chain evidence, a bounded disposable-copy tamper drill, and a signed or independently retained chain-head anchor that detects tail truncation or full-chain recomputation. |
