...
| ALV-NET-003 | High | The affected `yamux 0.12.1` path is locally remediated at `80dc72b5e54a`: a reviewed compatibility adapter retains the 32-stream cap while resolving only fixed `yamux 0.13.10`. A follow-up applies it to the independently locked fuzz workspace. Immutable GitHub and Dependabot verification are pending. | The main, native-helper, and fuzz Cargo graphs exclude `yamux 0.12.1`; the malformed-frame regression, multi-node P2P tests, workspace tests, strict Clippy, bounded fuzz runs, and fresh dependency scans pass; see `YAMUX_BACKPORT_EVIDENCE_2026-07-30.md`. | Green same-commit GitHub checks, successful Dependabot re-check, and eventual replacement of the temporary backport by a reviewed published rust-libp2p upgrade. |
